                    ...
                    $(me.sections).find('.tbody .rz').each(function(index, element) {
                        ...
                        if ($(this).text() == '离店') {
                            ...
                            if($('.lidian_hover').text() != ""){
                                var NumDate = $('.lidian_hover').text().replace(/[^0-9]/ig,"");//天数
                                $('.NumDate').text(NumDate);
                                
								var price=$("#house-price").text().split("元");//房间价格
								var NumHouse=$("#need-number").val();//房间数目
								
								$("#all-money").text(NumDate*NumHouse*price[0]).append('.00');
								$("#pay-money").text(NumDate*NumHouse*price[0]).append('.00');
                            }
